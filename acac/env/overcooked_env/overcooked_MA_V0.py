...
from .overcooked_V0 import Overcooked_V0
from .mac_agent import MacAgent
from .map_utils import make_counter_seq
import random
...
class Overcooked_MA_V0(Overcooked_V0):
    ...
    def __init__(self, grid_dim, task, rewardList, map_type="A", n_agent=2, 
        ...
        super().__init__(grid_dim, task, rewardList, map_type, n_agent, obs_radius, mode, debug, rand_start, custom, obj, n_knife, n_plate)
        self.macroAgent = []
        self._createMacroAgents()
        self.macroActionItemList = []
        self._createMacroActionItemList()
        self.macroAction2Item = {}
        self.map_type = map_type
        self.n_plate = n_plate
        self.n_knife = n_knife
        if map_type == "A":
            self.macroActionName = ["stay"]
            for o in self.object :
                self.macroActionName.append("get "+o)
                for f in self.food:
                    if f.rawName == o :
                        self.macroAction2Item["get "+o] = f
                        break
            for i in range(0,self.n_plate) :
                self.macroActionName.append("get plate "+str(i+1))
                self.macroAction2Item["get plate "+str(i+1)] = self.plate[i]
            
            for i in range(0,self.n_knife) :
                self.macroActionName.append("go to knife "+str(i+1))    
            self.macroActionName+=["delivery", "chop", "right", "down", "left", "up"]
            ...
            for i in range(0,self.n_knife) :
                self.macroActionName.append("go to knife "+str(i+1))    
            self.macroActionName+=["delivery", "chop", "go to counter", "right", "down", "left", "up"]
        
        
    
        self.action_space = spaces.Discrete(len(self.macroActionName))
                            
        self.counterSequence = make_counter_seq(size=self.xlen, map_type=self.mapType)
                
    def _update_actiontoitem(self) :
        self.macroAction2Item = {}
        if self.map_type == "A":
            self.macroActionName = ["stay"]
            for o in self.object :
    ...
    def _findPOitem(self, agent, macro_action):
        ...
        macro_action_name = self.macroActionName[macro_action]
        object_name = self._macroActionName_to_objectName(macro_action_name)
        idx = self.idx_in_obs[object_name]
    ...
    def _checkMacroActionDone(self):
        ...
        for idx, agent in enumerate(self.agent):
            if not self.macroAgent[idx].cur_macro_action_done:
                ...
                if 'knife' in self._macroActionName_to_objectName(self.macroActionName[macro_action]) and not agent.holding:
                    target_x, target_y = self._findPOitem(agent, macro_action)
                    if self._calDistance(agent.x, agent.y, target_x, target_y) == 1:
                        self.macroAgent[idx].cur_macro_action_done = True
                
                elif self._macroActionName_to_objectName(self.macroActionName[macro_action]) in self.object:
                    target_x, target_y = self._findPOitem(agent, macro_action)
                    ...
                    if self._calDistance(agent.x, agent.y, target_x, target_y) == 1:
                        for knife in self.knife:
                            if knife.x == target_x and knife.y == target_y:
                                ...
                                food = self._findItem(target_x, target_y, self._macroActionName_to_objectName(self.macroActionName[macro_action]))
                                if not food.chopped:
                                    self.macroAgent[idx].cur_macro_action_done = True
                                    break
                    ...
                    for i in self.counterSequence:
                        if self.item_name[agent.pomap[i][target_y]] == "counter":
                            target_x = i
                ...
                if self._macroActionName_to_objectName(self.macroActionName[macro_action]) in self.object\
                    or 'plate' in self._macroActionName_to_objectName(self.macroActionName[macro_action]):
                        target_x, target_y = self._findPOitem(agent, macro_action)
                        item = self.macroAction2Item[self.macroActionName[macro_action]]
                            
                        if target_x != item.x or target_y != item.y:
                            self.macroAgent[idx].cur_macro_action_done = True

    def _macroActionName_to_objectName(self,macro_action_name) :
        obejct_name = macro_action_name.replace('get ','').replace('go to ','')
        return obejct_name 
        ...
        for idx, agent in enumerate(self.agent):
            ...
            if macro_action == 0:
                ...
                for action in range(4):
                    ...
                    new_name = self.item_name[self.map[new_x][new_y]] 
                    if new_name == "knife":
            ...
            elif self.macroActionName[macro_action] == "deliver" and agent.x == 1 and agent.y == 1 and self.item_name[agent.pomap[2][1]] == "agent":
                ...
                #print(ITEMNAME,agent.pomap[new_x][new_y])
                
                if self.item_name[agent.pomap[new_x][new_y]] == "space":
                #if self.item_name[agent.pomap[new_x][new_y]] == "space" or self.item_name[agent.pomap[new_x][new_y]] == "counter" :
                    primitive_action = action
                else:
                    primitive_action = ACTIONIDX["stay"]
                ...
                if target_x == 1 and target_y == 0 and agent.x == 3 and agent.y == 1 and self.item_name[agent.pomap[2][1]] == "agent":
                    primitive_action = ACTIONIDX["right"]
               
                elif self.item_name[agent.pomap[target_x][target_y]] == "agent" \
                    ...
                    if self._calDistance(agent.x, agent.y, target_x, target_y) == 1:
                        ...
                        if 'plate' in self._macroActionName_to_objectName(self.macroActionName[macro_action]) and agent.holding:
    ...
    def _navigate(self, agent, target_x, target_y):
        ...
        while not q.empty():
            ...
            for action in range(4):
                ...
                new_name = self.item_name[agent.pomap[new_x][new_y]] 
