...
from .overcooked_V1 import Overcooked_V1
...
class Overcooked_MA_V1(Overcooked_V1):
    ...
    def __init__(self, grid_dim, task, rewardList, map_type="A", n_agent=2, obs_radius=2, mode="vector", debug=False, rand_start=False):  
        ...
        super().__init__(grid_dim, task, rewardList, map_type, n_agent, obs_radius, mode, debug, rand_start)
        self.macroAgent = []
    ...
    def run(self, macro_actions):
        ...
        info = {'cur_mac': cur_mac, 'mac_done': mac_done, "actions": actions}
        # cur_mac: actual macro action done at the current step
        # mac_done: macro action done at the current step = macro action start at the next step
        return  self._get_macro_obs(), rewards, terminate, info

    def _checkCollision(self, info):
        for idx in info["collision"]:
            # print(f"Agent {idx+1} Macro-action Done: collision")
            ...
            if not self.macroAgent[idx].cur_macro_action_done:
                ...
                if self.macroActionName[macro_action] in ["go to knife 1", "go to knife 2"] and not agent.holding:
                    ...
                    if self._calDistance(agent.x, agent.y, target_x, target_y) == 1:
                        self.macroAgent[idx].cur_macro_action_done = True
                        # print(f"Agent {idx+1} Macro-action Done @ _checkMaroActionDone: go to knife & not holding & dist == 1 ")
                    ...
                    if self._calDistance(agent.x, agent.y, target_x, target_y) == 1:
                        for knife in self.knife:
                            if knife.x == target_x and knife.y == target_y:
                                ...
                                if not food.chopped:
                                    # print(f"Agent {idx+1} Macro-action Done @ _checkMaroActionDone: go to food & food on knife & not chopped")
                                    self.macroAgent[idx].cur_macro_action_done = True
                                    break
                elif self.macroActionName[macro_action] == "deliver" and not agent.holding:
                    target_x, target_y = self._findPOitem(agent, macro_action)
                    if self._calDistance(agent.x, agent.y, target_x, target_y) == 1:
                        # print(f"Agent {idx+1} Macro-action Done @ _checkMaroActionDone: deliver & not holding & dist == 1")
                        self.macroAgent[idx].cur_macro_action_done = True
                    ...
                    if findEmptyCounter:
                        if self._calDistance(agent.x, agent.y, target_x, target_y) == 1:
                            # print(f"Agent {idx+1} Macro-action Done @ _checkMaroActionDone: empty counter & dist == 1")
                            self.macroAgent[idx].cur_macro_action_done = True
                    else:
                        # print(f"Agent {idx+1} Macro-action Done @ _checkMaroActionDone: no empty counter")
                        self.macroAgent[idx].cur_macro_action_done = True
                ...
                if self.macroActionName[macro_action] in ["get tomato", "get lettuce", "get onion"]\
                    or self.macroActionName[macro_action] in ["get plate 1", "get plate 2"]:
                        ...
                        # print(f"Agent {idx+1} Macro-action Done @ _checkMaroActionDone: target: {target_x}, {target_y} & item: {item.x}, {item.y}")
                        if target_x != item.x or target_y != item.y:
                            # print(f"Agent {idx+1} Macro-action Done @ _checkMaroActionDone: item location is different: target: {target_x}, {target_y} & item: {item.x}, {item.y}")
                            self.macroAgent[idx].cur_macro_action_done = True
    ...
    def _computeLowLevelActions(self, macro_actions):
        ...
        for idx, agent in enumerate(self.agent):
            if self.macroAgent[idx].cur_macro_action_done:
                # print(f"len(self.macroAgent): ", len(self.macroAgent))
                # print(f"self.macroAgent[{idx}].cur_macro_action: ", self.macroAgent[idx].cur_macro_action)
                # print(f"len(macro_actions): ", len(macro_actions))
                # print(f"macro_actions[{idx}]: ", macro_actions[idx])
            ...
            if macro_action == 0:
                # print(f"Agent {idx+1} Macro-action Done: macro_action == 0")
                self.macroAgent[idx].cur_macro_action_done = True
                        ...
                        if isinstance(knife.holding, Food):
                            if not knife.holding.chopped:
                                ...
                                if self.macroAgent[idx].cur_chop_times >= 3:
                                    # print(f"Agent {idx+1} Macro-action Done: cur_chop_times >= 3")
                                    self.macroAgent[idx].cur_macro_action_done = True
                                    self.macroAgent[idx].cur_chop_times = 0
                                break
                if primitive_action == ACTIONIDX["stay"]:
                    # print(f"Agent {idx+1} Macro-action Done: self.macroActionName[macro_action] == 'chop' & primitive_action == ACTIONIDX['stay']")
                    self.macroAgent[idx].cur_macro_action_done = True
                ...
                if findEmptyCounter:
                    ...
                    if self._calDistance(agent.x, agent.y, target_x, target_y) == 1:
                        # print(f"Agent {idx+1} Macro-action Done: (B or C) & go to counter & dist == 1")
                        self.macroAgent[idx].cur_macro_action_done = True
                else:
                    primitive_action = ACTIONIDX["stay"]
                    # print(f"Agent {idx+1} Macro-action Done: (B or C) & go to counter & no empty counter")
                    self.macroAgent[idx].cur_macro_action_done = True
            elif macro_action >= self.macroActionName.index("right"):
                # print(f"Agent {idx+1} Macro-action Done: primitive action selected")
                self.macroAgent[idx].cur_macro_action_done = True
                action = macro_action - self.macroActionName.index("right")
                new_x = agent.x + DIRECTION[action][0]
                new_y = agent.y + DIRECTION[action][1]
                if ITEMNAME[agent.pomap[new_x][new_y]] == "space":
                #if ITEMNAME[agent.pomap[new_x][new_y]] == "space" or ITEMNAME[agent.pomap[new_x][new_y]] == "counter" :
                    primitive_action = action
                else:
                    primitive_action = ACTIONIDX["stay"]
            else:
                    ...
                    if (target_x >= agent.x - self.obs_radius and target_x <= agent.x + self.obs_radius and target_y >= agent.y - self.obs_radius and target_y <= agent.y + self.obs_radius) \
                        ...
                        for plate in self.plate:
                            if plate.x == target_x and plate.y == target_y:
                                primitive_action = ACTIONIDX["stay"]
                                # print(f"Agent {idx+1} Macro-action Done: get food in obs range & food in plate")
                                self.macroAgent[idx].cur_macro_action_done = True
                                inPlate = True
                                break
                if inPlate:
                ...
                if target_x == 1 and target_y == 0 and agent.x == 3 and agent.y == 1 and ITEMNAME[agent.pomap[2][1]] == "agent":
                    ...
                    # print(f"Agent {idx+1} Macro-action Done: other agent at the target & in the obs range.")
                    self.macroAgent[idx].cur_macro_action_done = True
                else:
                    primitive_action = self._navigate(agent, target_x, target_y)
                    if primitive_action == ACTIONIDX["stay"]:
                        # print(f"Agent {idx+1} Macro-action Done: all other case & stay")
                        self.macroAgent[idx].cur_macro_action_done = True
                    if self._calDistance(agent.x, agent.y, target_x, target_y) == 1:
                        # print(f"Agent {idx+1} Macro-action Done: all other case & dist == 1")
                        self.macroAgent[idx].cur_macro_action_done = True
                        if self.macroActionName[macro_action] in ["get plate 1", "get plate 2"] and agent.holding:
